...
const [base,expansion,approval,history,legacy,repertorios,publicacoes,cultural,themes]=await Promise.all([
  ...
  read('data/historico-migracao-legado-lote6-v1.json'),
...
for(const id of expected){
  ...
  assert(migratedMap.get(id)?.lote==='legado-lote6-v1',`${id} não está registrado no sexto lote.`);
  assert(repertoryIds.has(id),`${id} perdeu o registro histórico original.`);
  assert(historyIds.has(id),`${id} não possui cópia histórica do sexto lote.`);
...
for(const id of expected){
  ...
  assert(item.origem_migracao?.lote==='legado-lote6-v1',`${id} não registra o sexto lote.`);
  assert(item.origem_migracao?.historico_ref==='data/historico-migracao-legado-lote6-v1.json',`${id} não aponta para o histórico correto.`);
...
console.log('Sexto lote migrado com histórico, aprovação, contrato e total público preservados.');
