...
for(const id of expected){assert(!legacyIds.has(id),`${id} ainda aparece como publicado_legado.`);assert(migratedMap.get(id)?.lote==='legado-lote5-v1',`${id} não está registrado no quinto lote.`);assert(repertoryIds.has(id),`${id} perdeu o registro histórico original.`);assert(historyIds.has(id),`${id} não possui cópia histórica do quinto lote.`);assert(culturalIds.has(id),`${id} não chegou à base cultural canônica.`);}
...
assert(cultural.length===26,'A base cultural final deve conter 26 repertórios.');
...
for(const id of expected){const item=cultural.find(entry=>entry.id===id);assert(item.status==='publicado',`${id} não está publicado.`);assert(item.aprovacao?.aprovado_por==='Luiz Jácomo',`${id} não preserva a aprovação nominal.`);assert(item.origem_migracao?.lote==='legado-lote5-v1',`${id} não registra o quinto lote.`);assert(item.origem_migracao?.historico_ref==='data/historico-migracao-legado-lote5-v1.json',`${id} não aponta para o histórico correto.`);assert(Array.isArray(item.autores)&&item.autores.length===0,`${id} criou relação autoral automática.`);}
const junipero=cultural.find(item=>item.id==='CUL-0007');assert(junipero.leitura_sociosofia.includes('dimensão amorosa e queer'),'CUL-0007 perdeu a dimensão afetiva e queer.');assert(junipero.cuidado_pedagogico.includes('Não apresentar a transferência de consciência como fato científico'),'CUL-0007 apresentou ficção como fato científico.');
const entireHistory=cultural.find(item=>item.id==='CUL-0008');assert(entireHistory.leitura_sociosofia.includes('Transformar a experiência em arquivo não elimina a interpretação'),'CUL-0008 confundiu registro e verdade.');assert(entireHistory.cuidado_pedagogico.includes('controle coercitivo'),'CUL-0008 perdeu o cuidado sobre controle íntimo.');
const merits=cultural.find(item=>item.id==='CUL-0009');assert(merits.leitura_sociosofia.includes('contestação em produto'),'CUL-0009 perdeu a captura da crítica.');assert(merits.cuidado_pedagogico.includes('alegoria')&&merits.cuidado_pedagogico.includes('sexualização'),'CUL-0009 perdeu os cuidados pedagógicos.');
...
console.log('Quinto lote continua íntegro após o encerramento do legado.');
