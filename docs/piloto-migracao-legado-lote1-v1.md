# Piloto de migração do acervo legado — lote 1 v1
...
O estado transitório `publicado_legado` preserva a fachada pública que já existia antes da migração canônica, sem converter os antigos estados `rascunho` e `aguardando revisão` em aprovação editorial plena.

A implementação proposta separa duas coisas:

- **workflow editorial legado**, mantido em `data/repertorios.json`;
- **estado público observado**, registrado em `data/publicacao-legado.json`.

O registro contém os 33 itens historicamente visíveis: 7 cards `DAD` e 26 cards `CUL`. Novos rascunhos não se tornam públicos por estarem em `data/repertorios.json`; precisam de inclusão deliberada no registro ou de publicação pelo contrato canônico.

O carregador passa a aceitar apenas:

- `status: publicado`, para cards canônicos;
- IDs registrados como `publicado_legado`, durante a migração.

Se o registro transitório não puder ser carregado, os itens legados não são expostos por suposição. As publicações canônicas continuam disponíveis.

## 2. Preservações do piloto

- os IDs e endereços permanecem os mesmos;
- os 33 repertórios já visíveis continuam na fachada pública;
- nenhuma proposta deste documento substitui o texto vigente;
...
- `data/publicacao-legado.json` — estado público transitório dos 33 itens;
- `data/propostas-migracao-legado-lote1-v1.json` — três projeções propostas, ainda fora da renderização;
- `site-shared.js` — portão público baseado em `publicado` ou `publicado_legado`;
- `scripts/test-legado-publico.mjs` — integridade do registro e das propostas;
- `audit/legacy-publication-browser.mjs` — conferência da fachada com 33 legados e 5 cards canônicos.

## 7. Decisão esperada após a revisão

A revisão pode aprovar separadamente:

1. o portão `publicado_legado`;
2. `DAD-0004`;
3. `DAD-0007`;
4. `CUL-0003`.

A aprovação do portão não aprova automaticamente os textos migrados. A aprovação de um card não aprova os demais.
