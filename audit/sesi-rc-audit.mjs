...
const targets=[
  {id:'filosofia-1ano',path:'/alunos/filosofia-1ano/',chapters:10,subject:'Filosofia',year:'1º ano'},
...
async function auditAnnual(browser,target,device){
  ...
  try{
    ...
    const movementIds=data.chapters.flatMap(c=>c.movements.map(m=>m.id));
    if(new Set(movementIds).size!==movementIds.length) addFailure(result,'Há IDs de movimentos duplicados.');
    ...
    const query=target.subject==='Filosofia'?'Kant':'Marx'; await page.locator('#busca').fill(query); await page.waitForTimeout(250);
...
md+=`## Percursos testados\n\n${targets.map(t=>`- ${t.subject} — ${t.year}: ${t.chapters} capítulos — desktop e celular`).join('\n')}\n\n`;
