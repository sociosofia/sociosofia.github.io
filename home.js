import {TEMAS,loadItems,loadEntities,inTheme,text,norm,esc} from './site-shared.js';
import {prepareHeader,buildShell} from './home-shell.js';
import {card,feature,entityDetail,entityNames,entitySearchCard} from './home-cards.js';

prepareHeader();buildShell();
const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
let items=[],entities={autores:[],conceitos:[]},state={tema:{dados:'',cultura:''},tipo:'conceito',entidade:'',aberto:''};

init();
async function init(){
  menu();events();
  try{[items,entities]=await Promise.all([loadItems(),loadEntities()]);renderAll();params();}
...
function renderAll(){
  ...
  $('#contador-conceitos').textContent=`${entityNames(items,entities,'conceito').length+entityNames(items,entities,'autor').length} entradas`;
...
function renderThemes(bloco){
  ...
  const buttons=[`<button class="topic-button ${!sel?'active':''}" data-theme=""><strong>Todos</strong><small>${base.length}</small></button>`];
  TEMAS.forEach(t=>{const n=base.filter(i=>inTheme(i,t)).length;buttons.push(`<button class="topic-button ${sel===t.id?'active':''}" data-theme="${t.id}" title="${esc(t.descricao)}"><strong>${esc(t.nome)}</strong><small>${n}</small><span>${esc(t.menores.slice(0,3).join(' · '))}</span></button>`);});
  el.innerHTML=buttons.join('');
}

function renderPortal(bloco){
  const sel=state.tema[bloco],base=items.filter(i=>i.bloco===bloco),tema=TEMAS.find(t=>t.id===sel),out=sel?base.filter(i=>inTheme(i,tema)):base;
  const titulo=tema?.nome||'Todos os repertórios';
  const apoio=tema?`<p class="theme-description">${esc(tema.descricao)}</p><div class="minor-theme-list">${tema.menores.map(x=>`<span>${esc(x)}</span>`).join('')}</div>`:'';
  $(`#lista-${bloco}`).innerHTML=`<div class="portal-result-heading"><div><h3>${esc(titulo)}</h3>${apoio}</div><p>${out.length} resultado${out.length===1?'':'s'}</p></div>${out.length?`<div class="card-grid">${out.map(card).join('')}</div>`:'<p class="empty-state">Ainda não há repertórios neste grande tema. O eixo já está reservado para novas fichas.</p>'}`;
}

function renderEntities(){const q=norm($('#busca-entidades')?.value),names=entityNames(items,entities,state.tipo).filter(n=>norm(n).includes(q));$('#lista-entidades').innerHTML=names.map(n=>`<button class="entity-chip ${state.entidade===n?'active':''}" data-entity-type="${state.tipo}" data-entity-name="${esc(n)}">${esc(n)}</button>`).join('')||'<p class="portal-prompt">Nenhuma entrada encontrada.</p>';}
function promptEntity(){$('#detalhe-entidade').innerHTML='<p class="portal-prompt">Escolha uma entrada para ver a ficha editorial e suas conexões com os outros dois blocos.</p>';}
function selectEntity(tipo,nome){state.tipo=tipo;state.entidade=nome;openPortal('conceitos',false);$$('[data-entity-type]').forEach(x=>x.classList.toggle('active',x.dataset.entityType===tipo));renderEntities();$('#detalhe-entidade').innerHTML=entityDetail(items,entities,tipo,nome);$('#detalhe-entidade').scrollIntoView({behavior:'smooth',block:'nearest'});}
...
function search(q,scroll=true){
  const s=norm(q.trim()),out=items.filter(i=>!s||text(i).includes(s));
  const matchedAuthors=entities.autores.filter(p=>!s||norm(JSON.stringify(p)).includes(s));
  const matchedConcepts=entities.conceitos.filter(p=>!s||norm(JSON.stringify(p)).includes(s));
  const total=out.length+matchedAuthors.length+matchedConcepts.length;
  $('#repertorios').hidden=false;
  $('#resumo-busca').textContent=q.trim()?`${total} resultado${total===1?'':'s'} para “${q.trim()}”.`:`${total} entradas disponíveis.`;
  $('#lista-resultados').innerHTML=[...out.map(card),...matchedAuthors.map(p=>entitySearchCard('autor',p)),...matchedConcepts.map(p=>entitySearchCard('conceito',p))].join('');
  $('#sem-resultados').hidden=total>0;if(scroll)$('#repertorios').scrollIntoView({behavior:'smooth'});
}
